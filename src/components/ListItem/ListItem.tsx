...
/**
 * ListItem component- a component for every item in the list
 * @param props item, index, openDetail, addToWatchList, removeFromWatchList, watchedList
 * @returns a Listitem Component
 */
export const ListItem: React.FC<Partial<ListItemProps>> = (props) => {
    ...
    return (
        <li key={item?.id} className="flex p-1 border m-1 rounded">
            ...
            <Button
                className="ml-auto"
                onClick={() => openDetail && index && openDetail(index)}
                label="Show Details"
            />
            <Button
                onClick={() =>
                    watchedList
                        ...
                        : addToWatchList && index && addToWatchList(index)
                }
